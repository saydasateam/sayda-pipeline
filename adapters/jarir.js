...
(function(){
  ...
  S.adapters.jarir = {
    ...
    async check(rows) {
      return S.pool(rows, 4, async r => { const { status, text } = await S.fetchText(r.url.replace('https://www.jarir.com', ''));
        const m = text.match(/"price"\s*:\s*"([\d.]+)"[\s\S]{0,80}?"availability"\s*:\s*"([^"]+)"/);
        return { id: r.id, found: status === 200 && !!m, live: m ? +m[1] : null, buyable: !!m && /InStock/.test(m[2]), stock: m && /InStock/.test(m[2]) ? 99 : 0 }; });
    },
