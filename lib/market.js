...
/** The English product name the STORE itself uses, read from the longest hyphenated segment of the
 *  product URL. Toy and kids rows carry Arabic names and no model code, so the slug is the only
 *  Latin identity they have — and it is the store's own words, not our paraphrase. The trailing
 *  numeric id is dropped; everything else is kept for rules/match.js to require word by word. */
function urlParts(row) { return String(row.url || '').split('?')[0].split('/').filter(Boolean).slice(2); }
function slugSeg(row) {
  const parts = urlParts(row);
  return parts.filter(x => /[a-z]/i.test(x) && x.includes('-')).sort((a, b) => b.length - a.length)[0] || '';
}
function enNameOf(row) { return slugSeg(row).replace(/-\d{4,}$/, '').replace(/-/g, ' ').trim(); }

/** Brand for a row that has none: the store's own brand segment in the URL, which is the segment
 *  BEFORE the product slug (firstcry.sa/leapfrog/<slug>/<id>/product-detail), or the opening word
 *  of the slug when there is no such segment (dabdoob: clementoni-new-york-puzzle-2000-pcs-39904).
 *
 *  Not `brandOf` on the slug text: that scans the whole name for any known brand word and picked
 *  "marshall" out of "Chase Marshall Skye Rubble" on a Melissa & Doug puppet set — the Marshall
 *  speaker brand, from a Paw Patrol character's name. Position is the signal here, not a word list.
 */
const SLUG_NOISE = new Set(['product', 'product-detail', 'ar-sa', 'en-sa', 'p', 'c', 'sa', 'ar', 'en', 'buy']);
function brandFromUrl(row) {
  const parts = urlParts(row), seg = slugSeg(row);
  const i = parts.indexOf(seg);
  const before = i > 0 ? parts[i - 1] : '';
  if (before && !SLUG_NOISE.has(before.toLowerCase()) && /[a-z]/i.test(before) && !/^\d+$/.test(before)) {
    return before.replace(/-/g, ' ').toLowerCase();
  }
  const words = enNameOf(row).split(/\s+/).filter(Boolean);
  return words.length ? words[0].toLowerCase() : '';
}

function queryFor(row) {
  const brand = brandOf(row) || brandFromUrl(row);
  const models = String(row.model || '').trim() ? [String(row.model).trim()] : modelTokens(row.name);
  if (!brand) return null;
  // No model code anywhere — the name path (rules/match.js). Query the store's own English name,
  // trimmed to the first few words: a 20-word slug returns nothing from a shopping index.
  if (!models.length) {
    const en = enNameOf(row);
    if (!en) return null;
    const brandWords = new Set(String(brand).toLowerCase().split(' '));
    const words = en.toLowerCase().split(/\s+/).filter(w => w && !brandWords.has(w));
    if (words.length < 2) return null;
    // The SAME six words go into the query and into the match: a full slug can run to twenty words
    // ("… with bbc videos world geography learning for children 5 years"), and requiring all of them
    // rejects every real listing, which reads as "no comparator" rather than as a filter that is too
    // strict. Six leading words, minus stopwords in rules/match.js, is what has to be present.
    const head = words.slice(0, 6);
    const q = [brand, ...head].join(' ').replace(/\s+/g, ' ').trim();
    return { q, brand, model: '', enName: head.join(' '), variant: variantOf(row.name), size: sizeOf(row.name) };
  }
...
module.exports = { load, save, isFresh, due, queryFor, put, storeOf, brandOf, brandFromUrl, enNameOf, modelTokens, variantOf, sizeOf, DIR, REFRESH_DAYS, PRICE_MOVE };
